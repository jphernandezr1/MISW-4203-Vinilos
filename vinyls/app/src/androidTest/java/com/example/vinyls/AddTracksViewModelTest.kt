...
class AddTracksViewModelTest {
    ...
    fun submitTracks_rejectsDuplicateNames() = runBlocking {
        ...
        Assert.assertEquals(
            "Track already exists in this album",
            viewModel.uiState.first().errorMessage
        )
    }
    ...
    fun submitTracks_addsTrackAndReturnsSuccess() = runBlocking {
        ...
        Assert.assertEquals("Tracks added successfully", state.successMessage)
        Assert.assertEquals(3, state.album?.tracks?.size)
        Assert.assertFalse(state.isSaving)
    }
